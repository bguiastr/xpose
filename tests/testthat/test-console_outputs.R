...
test_that('Check print.xpose_data returns a proper message', {
  ...
  expect_snapshot(capture_output(print(xpdb_ex_pk)))
  expect_snapshot(capture_output(print(xpdb_modified)))
})

test_that('Check prm_table returns a proper message', {
  local_edition(3) 
  
  expect_snapshot(capture_output(prm_table(xpdb_ex_pk, .problem = 1)))
  expect_snapshot(capture_output(prm_table(xpdb_ex_pk, transform = FALSE)))
})

test_that('Check summary.xpose_data returns a proper message', {
  local_edition(3) 
  
  expect_snapshot(capture_output(summary(xpdb_ex_pk)))
})

test_that('Check list_vars returns a proper message', {
  local_edition(3) 
  
  expect_snapshot(capture_output(list_vars(xpdb_ex_pk, .problem = 1)))
})

test_that('Check list_data returns a proper message', {
  local_edition(3) 
  
  expect_snapshot(capture_output(list_data(xpdb_ex_pk)))
})

test_that('Check list_files returns a proper message', {
  local_edition(3) 
  
  expect_snapshot(capture_output(list_files(xpdb_ex_pk)))
})

test_that('Check list_special returns a proper message', {
  local_edition(3) 
  
  expect_snapshot(capture_output(list_special(xpdb_ex_pk %>% vpc_data(quiet = TRUE))))
})
